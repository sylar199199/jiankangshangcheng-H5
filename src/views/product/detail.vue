<template>
  <div class="container" v-if="showData">
    <div class="flexContent">
      <div :class="!isWx?'product-detail-box marginTop':'product-detail-box'">
        <div class="swiperBox">
          <div class="swiper-inner">
            <!-- swiper -->
            <swiper :options="swiperOption" ref="mySwiper" v-if="content.imgList && content.imgList.length>1">
            ...
            <div class="banner" v-for="(item,index) in content.imgList"
                 v-if="content.imgList && content.imgList.length == 1">
              <a>
                <img :src="item.imgUrl"/>
              </a>
            </div>
        ...
        <div class="product-package-box">
          <div class="guige">
            <a class="mint-cell mint-field" v-if="rightsFlag != '01'">
        ...
        <div class="productDescription">
          <div class="tabBox">
            <div v-show="$route.params.type != 1"  :class="tabActive==1?'activeTabtitle':'tabTitle'" @click="tabActive=1">图文详情</div>
            <div class="lineBox"></div>
            <div v-show="$route.params.type != 1" :class="tabActive==2?'activeTabtitle':'tabTitle'" @click="tabActive=2">常见疑问</div>
          </div>
      ...
      <div v-if="checkProtocol">
        <a class="mint-cell mint-field">
          <div class="mint-cell-wrapper">
            <div class="mint-cell-title">
                        <span class="mint-cell-text">
                          <img class="confirmIcon" v-show="noSelect" @click="changenoSelect(false)"
                               src="../../assets/image/weixuan.png">
                          <img class="confirmIcon" v-show="!noSelect" @click="changenoSelect(true)"
      ...
      <div class="detail-footer flexBetween">
        <div class="service-bar" @click="popupService=true">
          <img class="img-con" src="../../assets/image/kefu.png" alt="电话客服"/>
        </div>
        <div class="buy-bar" @click="goToNext()" v-if="rightsFlag != '01'">
          立即购买
        </div>
        <div class="buy-bar" v-show="useShow" @click="goToSelect()" v-if="rightsFlag == '01'">
          立即使用
        </div>
        <div class="buy-bar" v-show="!useShow" @click="goLookRead()" v-if="rightsFlag == '01'">
          去查阅
        </div>
      </div>
    </div>
    <mt-popup v-model="popupxuzhi" position="bottom" class="popGuige">
      <div class="poptitlebox">
        <span class="popTitle">用户信息采集协议</span>
        <div class="popup-close" @click="popupxuzhi=false">
        </div>
      </div>
      <div class="popContent popContentone" v-html="purchaseNotes.agreementHtml">
      </div>
    </mt-popup>
        ...
        <div class="popContent">
          ...
          <div class="flexBox">
            <div class="guigeBox" v-for="item in skuList" :key='item.stock'>
              <span v-if="parseInt(item.stock)+parseInt(item.cardStock) != 0"
                    :class="item.isSelect?'selectguigetext':'guigetext'" @click="seletGuige(item)">{{item.plan.productPlanName}}</span>
    ...
    <mt-popup v-model="ServiceSeletpopup" position="bottom" class="select-serve">
      <div class="poptitlebox">
        <span class="popTitle">选择服务</span>
        <div class="popup-close" @click="ServiceSeletpopup=false">
        </div>
      </div>
      <div class="tip">
        尊敬的会员，您将可以享用以下全部权益，请分次进行预约：
      </div>
      <div class="xian"></div>
      <ul>
        <li v-for="item in userSpecList" :key="item.plan.id">
          <div class="left">
            <img v-if="item.plan.serviceTypeId == 1" src="../../assets/image/equity/menzhen.png" alt="">
            <img v-if="item.plan.serviceTypeId == 2" src="../../assets/image/equity/peizhen.png" alt="">
            <img v-if="item.plan.serviceTypeId == 3" src="../../assets/image/equity/xietiao.png" alt="">
            <img v-if="item.plan.serviceTypeId == 4" src="../../assets/image/equity/yuyue.png" alt="">
            <img v-if="item.plan.serviceTypeId == 5" src="../../assets/image/equity/erzhen.png" alt="">
            <img v-if="item.plan.serviceTypeId == 8" src="../../assets/image/equity/fubingzixun.png" alt="">
            <img v-if="item.plan.serviceTypeId == 9" src="../../assets/image/equity/fangan.png" alt="">
          </div>
          <div class="right">
            <div class="name-style">
              <div class="name-serve">{{item.plan.productPlanName}}</div>
              <div class="num-serve">剩余{{item.rightsPlan.totalTimes - item.rightsPlan.useTimes}}次</div>
            </div>

            <span class="button bcSuccsee"
                  v-if="item.rightsPlan.totalTimes && (item.rightsPlan.totalTimes - item.rightsPlan.useTimes) > 0"
                  @click="goForm(item.plan.serviceTypeId, item.id, item.plan.entityFlag, item.plan.productPlanShortName, item.auditFlag)">立即使用
            </span>
            <span v-if="!item.rightsPlan.totalTimes || (item.rightsPlan.totalTimes - item.rightsPlan.useTimes) == 0"
                  class="button bcCancel">已用完</span>
          </div>
        </li>
      </ul>
    </mt-popup>
    ...
    <mt-popup v-model="popupTip" popup-transition="popup-fade" class="popup-service">
      ...
      <div class="Padding30">
        你已经享有该服务，医生会进行电话预约，如有问题请拨打400电话进行查询
      </div>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      Util.sessionStorageUtil.clear("selectProduct" + this.$route.params.productId)
      ...
      this.noSelect = !Util.localStorageUtil.get(useId)
    },
    methods: {
      wxLogin() {
        ...
        if (timestamp && encode_string) {
          Service.login().wxlogin({
            ...
            if (response.errorCode == 0) {
              (this.$route.params.type && this.$route.params.type == 1) ? this.getEquityDeatils() : this.getDetail();
      ...
      goForm(typeId, specId, entityFlag, productPlanShortName, auditFlag) {
        let paramsCont = this.$route.params.contNo == 0 ? '' : this.$route.params.contNo;
        Service.goods().isUncompleted({}, specId, paramsCont).then((res) => {
          if(res.data){
            if(typeId == 4){
              Toast('您有此服务未完成的订单，请完成后再试!');
            } else {
              Toast('您有此服务未完成的订单，请完成后再试!');
            }
      ...
      changenoSelect(isSelect){
        this.noSelect = isSelect;
        const useId = Util.localStorageUtil.get("loginInfo").id
        if(!isSelect){
          Util.localStorageUtil.set(useId, !isSelect);
        }else{
          Util.localStorageUtil.clear(useId);
        }
      },
      goDenglu() {
      ...
      goOrderconfirm() {
        ...
        if (this.kucun == 0) {
          ...
          Util.sessionStorageUtil.set("selectKlProduct" + this.$route.params.productId, selectKlProduct);
      ...
      goLookRead() {
        if (this.noSelect && this.checkProtocol) {
          Toast('请勾选用户信息采集协议')
          return;
        }
        window.location.href = "https://mall.kunlunhealth.com/service/health-info"
      },
      goToSelect() {
        if (this.noSelect && this.checkProtocol) {
          Toast('请勾选用户信息采集协议')
        ...
        // 视频问诊
        if (this.userSpecList[0].plan.serviceTypeId == 7) {
          let paramsCont = this.$route.params.contNo == 0 ? '' : this.$route.params.contNo;
          Service.goods().videoPerson({
            contNo: paramsCont,
            spacId: this.userSpecList[0].id
        ...
        // 智能问诊
        if (this.userSpecList[0].plan.serviceTypeId == 6) {
          let paramsCont = this.$route.params.contNo == 0 ? '' : this.$route.params.contNo;
        ...
        if (this.userSpecList.length == 1) {
          let surplus = this.userSpecList[0].rightsPlan.totalTimes - this.userSpecList[0].rightsPlan.useTimes
          if(this.userSpecList[0].rightsPlan.totalTimes && surplus == 0){
            Toast('您的此服务无可用次数');
            return;
          }
          this.goForm(this.userSpecList[0].plan.serviceTypeId, this.userSpecList[0].id,
            this.userSpecList[0].plan.entityFlag, this.userSpecList[0].plan.productPlanShortName, this.userSpecList[0].auditFlag);
          return
        }
        let that = this;
        that.ServiceSeletpopup = true;
      },
      goHelpcenter() {
        this.$router.push({
          name: 'helpcenter',
      ...
      async getEquityDeatils() {
        ...
        if (this.$route.params.contNo == 0) {
          response = await Service.goods().getEquityDetail({}, this.id);
        } else {
          response = await Service.goods().getEquityContNODetail({}, this.id, this.$route.params.contNo);
        }
        if (response.errorCode == 0) {
          this.userSpecList = response.data.userSpecList;
          this.rightsFlag = '01';
          this.showData = true;
          this.content = response.data;
          this.skuList = this.content.skuList;
          ...
          this.purchaseLimit = response.data.purchaseLimit;
          this.collectionChannel = response.data.collectionChannel;
          this.auditFlag = response.data.userSpecList[0].auditFlag;
          this.checkProtocol = response.data.checkProtocol;
          if (this.purchaseLimit == 0) {
            this.showLimit = false;
          } else {
            this.showLimit = true;
          }
          if (response.data.userSpecList[0].plan.serviceTypeId == 10) {
            this.useShow = false;
          }
          for (let item of response.data.faqList) {
            item.iszhankai = false;
