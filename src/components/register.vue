			...
			<div class="popup-login-content">
				...
				<div class="pact-agree">
					...
					<span class="weui-agree-text">
						<a href="javascript:void(0);">本人已阅读并同意
							<span v-on:click="goDocDetail(1)">《平台服务协议》</span>
						</a>
					</span>
				</div>
...
<script>
    ...
    created() {
        ...
        console.log(this.$route.query.promotion)
    },
		methods: {
			goDocDetail(docId) {
			...
			getSmsCode() {
				if (!this.newUser.mobile) {
					...
					if (!this.unSms) {
                        Service.user().getusercode({
                        },this.newUser.mobile).then(response => {
                            if (response.errorCode == 0) {
                                this.hasUser = true;//已经存在用户
                                this.msgCallback()
                            }else if(response.errorCode == 5010){
                                Toast('尚未注册，前往实名认证页面')
                                this.$router.push({'name':'authentication'})
                                Store.commit("setIsShowRegister", false);
                                // this.getRegcode()
                            }
                        }, err => {
	...
	.popup-login.popup-login {
		...
		.popup-login-content {
			...
			.register-btn {
				...
				&.active {
					background-image: linear-gradient(90deg, #19b39d 0%, #19b39d  100%), linear-gradient(#ffffff, #ffffff);
